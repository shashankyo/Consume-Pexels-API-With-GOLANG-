...
type Photo struct {
	...
	PhotographUrl string      `json:"photograph_url"`
...
func (c *Client) requestDoWithAuth(method, url string) (*http.Response, err) {
	...
	times, err := strconv.Atoi(resp.Header.Get("X-Ratelimit-Reamining"))
	if err != nil {
		return resp, nil
	} else {
...
func (c *Client) SearchVideo(query, perPage, page int) (*VideoSearchResult, error) {
	fmt.Sprintf(VideoApi+"search?query=%s&per_page=%d&page=%d", query, perPage, page)
...
func (c *Client) PopularVideo(perPage, page int) (*PopularVideos, error) {
	fmt.Sprintf(VideoApi+"/popular?per_page=%d&page=%d", perPage, page)
	resp, err := c.requestDoWithAuth("GET", url)
	if err != nil {
		return nil, err
	}
